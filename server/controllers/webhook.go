...
func AddWebhook(ctx echo.Context) error {
	...
	// Atualiza o webhook na instância
	...
	if err := instances.Update(instance); err != nil {
		return ctx.JSON(500, echo.Map{"error": "erro ao salvar webhook"})
	}
...
func DeleteWebhook(ctx echo.Context) error {
	...
	instance.Webhook = nil

	if err := instances.Update(instance); err != nil {
		return ctx.JSON(500, echo.Map{"error": "erro ao remover webhook"})
	}
